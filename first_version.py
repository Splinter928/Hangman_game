...
def play(word1):
    word_completion = ['_' for _ in range(len(word1))]  # строка, содержащая символы _ на каждую букву задуманного слова
    guessed = False  # сигнальная метка
    guessed_letters = []  # список уже названных букв
    guessed_words = []  # список уже названных слов
    tries = 6  # количество попыток
    print('Давайте играть в угадайку слов!')
    print('Доступное количество попыток: %d' % tries)
    ...
    while tries > 0:
        ...
        if not char.isalpha():
            print('Вводите букву или слово целиком!')
            continue
        elif char in guessed_words or char in guessed_letters:
            print('Вы уже вводили такой вариант! Попробуйте что-то новенькое.')
            continue
        elif len(char) > 1 and char == word1:
#            print('%s - правильный ответ! Вы победили!' % char)
            guessed = True
            break
        elif len(char) > 1 and char != word1:
            tries -= 1
            guessed_words.append(char)
            print('Слово неверное! Осталось попыток: %d' % tries)
            print(display_hangman(tries))
            print('Слово: ', end='')
            print(*word_completion, sep='')
            continue
        elif len(char) == 1 and char not in word1:
            tries -= 1
            guessed_letters.append(char)
            print('Такой буквы в слове нет. Осталось попыток: %d' % tries)
            print(display_hangman(tries))
            print('Слово: ', end='')
            print(*word_completion, sep='')
            continue
        elif len(char) == 1 and char in word1:
            guessed_letters.append(char)
            ...
            if res == word1:
                ...
                continue
    if guessed:
        print('%s - правильный ответ! Вы победили!' % word)
    else:
        print('Слово было - %s. В этот раз не повезло :( Попробуйте снова!' % word)

game = 'да'
while game.lower() == 'да' or game.lower() == 'lf':
    word = get_word().upper()
    play(word)
    game = input('Хотите попробовать снова? Введите "да", чтобы продолжить.')
